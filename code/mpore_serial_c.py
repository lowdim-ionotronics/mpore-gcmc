...
#                self.state.ion_type = np.delete(self.state.ion_type, i_ion, axis=0)
    ...
    def thermalization(self, mu_c_kBT, restart_freq):
        for i in np.arange(self.state.mcparams.c_therm,self.state.mcparams.n_therm):
            self.mc_step(mu_c_kBT)
            if restart_freq and i%restart_freq==0 and i!=0 and i!=self.state.mcparams.c_therm:
                self.state.mcparams.c_therm += restart_freq
                pickle_write(self.state)       
        self.state.mcparams.c_therm += restart_freq
    ...
    def mc_simulate(self, mu_c_kBT, stat_freq, prefix, voltage, coord_dump=None, restart_freq=None):
        for i in np.arange(self.state.mcparams.c_sim,self.state.mcparams.n_sim):
            ...
            if i%stat_freq==0 and i!=self.state.mcparams.c_sim:
        ...
        pickle_write(self.state)       
        pickle_write(self.state, voltage)       

def pickle_write(state_obj, voltage=None):
    if voltage==None:
       fdata = open('./cont.restart','wb')
    else:
       fdata = open('v_'+str(voltage)+'.restart','wb')
    pickle.dump(state_obj, fdata, protocol=pickle.HIGHEST_PROTOCOL)
    fdata.close()

def pickle_load():
    fdata = open('./cont.restart','rb')
