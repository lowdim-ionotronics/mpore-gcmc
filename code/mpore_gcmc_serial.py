...
class cylinder(pore):
    ...
    def __init__(self, excl_width=0, length=0, eshift=0, wall_atom_radius=0, 
        ...
        self.volume_free= self.length*math.pi*(self.Rc-aion)**2
    ...
    def u2(self, r1, r2):
        ...
        rxy1 = np.sqrt(r1[:,0]**2+r1[:,1]**2); # phi1=atan2(r1[1],r1[0]); 
        rxy2 = np.sqrt(r2[0]**2+r2[1]**2); # phi2=atan2(r2[1],r2[0]); 
        phi = np.arctan2(r1[:,1],r1[:,0])-np.arctan2(r2[1],r2[0])
        return mplib_cyl_u2_vec(rxy1, rxy2, phi, dz, self.Rel) # * U_conv_coeff
    ...
    def box_bound(self, r, i_comp, aion):
        if (r[0]**2+r[1]**2 < ((self.Rc-aion)[i_comp])**2):
            return True
        else:
            return False
...
class slit(pore):
    ...
    def __init__(self, excl_width=0, length=0, eshift=0, wall_atom_radius=0,
        ...
        self.u1_interpol = interpolate.interp1d(r_data, U1_data)
        self.volume_free= self.length_x*self.length_y*(self.Rc-aion)*2
    ...
    def box_bound(self, r, i_comp, aion):
        if (np.abs(r[2]) < (self.Rc-aion)[i_comp]):
            return True
        else:
            return False
    ...
    def random_position(self, i_comp, aion):
        ...
        new_pos = np.array([self.length_x*np.random.rand(), 
                            ...
                            (self.Rc-aion)[i_comp]*2.*(np.random.rand()-0.5)])
        return new_pos
...
class state(object):
    ...
    def __init__(self, T = 300., epsr = 2.5, aion = np.array([2.5,2.5]), 
        ...
        self.r_xy = self.pore.Rc-self.aion # change pore functions to accept aion !!
...
#                self.state.ion_coord = np.delete(self.state.ion_coord, i_ion, axis=0)
#                self.state.ion_type = np.delete(self.state.ion_type, i_ion, axis=0)
    ...
    def thermalization(self, mu_c_kBT):
        for i in np.arange(self.state.mcparams.n_therm):
            self.mc_step(mu_c_kBT)
    
    def mc_simulate(self, mu_c_kBT, stat_freq, prefix, voltage, coord_dump=None, restart_freq=None):
        for i in np.arange(self.state.mcparams.n_sim):
            self.mc_step(mu_c_kBT)
            ...
            if restart_freq and i%restart_freq==0:
                pickle_write(self.state, prefix, voltage)       
        pickle_write(self.state, prefix, voltage)       

def pickle_write(state_obj, prefix, voltage):
    fdata = open(prefix+'_'+str(voltage)+'.restart','wb')
    pickle.dump(state_obj, fdata, protocol=pickle.HIGHEST_PROTOCOL)
    fdata.close()

def pickle_load(prefix, voltage):
    fdata = open(prefix+'_'+str(voltage)+'.restart','rb')
    state_obj = pickle.load(fdata)
    fdata.close()
    return state_obj
