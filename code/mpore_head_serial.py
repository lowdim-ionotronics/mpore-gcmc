...
parser.add_argument("-u", "--voltage", help="a single voltage (in V)", type=float, nargs='+', metavar="VAL", dest="u")
...
parser.add_argument("-R", "--restart", help="restart: 1 for resuming from restart file. output-prefix of the file to be read shoule be the same as the one provided", metavar="BOOL", type=int, dest="restart")
parser.add_argument("-srh", "--skip-restart-head", help="skip-restart-head: 1 to skip first voltage in the list (but the restart file read will correspond to the first voltage)", metavar="BOOL", type=int, dest="srh")
